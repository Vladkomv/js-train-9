...
function findIndexByProperty(arr, name) {
  ...
  const a2 = arr.findIndex(({ name: user }) => user === name);
  return a2 ? console.log(a2) : -1;
...
function findLastEvenNumber(arr) {
  ...
  const a3 = arr.findLast((el) => el % 2 === 0);
  return a3 ? console.log(a3) : "Парних чисел не знайдено";
...
function getPositiveArrayValues(arr) {
  ...
  const arrayIter = Array.from(iter);
  // За допомогою методу filter() отримуємо масив лише з додатніми значеннями.
  return arrayIter.filter((el) => el > 0); // Повертаємо масив додатніх значень.
}
...
function sumAndPrint(arr) {
  ...
  const sum = arr.reduce((accum, currentval) => accum + currentval);
...
function wordLengths(arr) {
  ...
  const arrresult = arr.map((el) => el.length);
...
function expandArrayByN(arr, n) {
  ...
  const expandedArray = arr.flatMap((el) => [...el.toString().repeat(n)]);
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((accum, currentval) => {
    return accum.length > currentval.length ? accum : currentval;
  });
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((el, index) => {
...
function capitalizeWords(arr) {
  ...
  const capitalizedArray = arr.map((el) => {
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce(
    (accum, currentval) => accum + currentval.price,
    0
  );
...
function findLastNegativeNumberIndex(arr) {
  ...
  const lastIndex = arr.findLastIndex((x) => x < 0);
